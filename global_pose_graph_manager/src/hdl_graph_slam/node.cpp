...
namespace hdl_graph_slam
{
  ...
  public:
    ...
    virtual void onInit()
    {
      ...
      // Keyframe Subscriber
      keyframe_sub_ = nh.subscribe("/orb_slam2_rgbd/keyframe", 64, &HdlGraphSlamNodelet::keyframe_callback, this);
      ...
      for (int vehicle_num = 0; vehicle_num < num_vehicle_; vehicle_num++)
      {
        ...
        odom2map_pubs.push_back(mt_nh.advertise<geometry_msgs::TransformStamped>(odom2map_topic, 16));

        std::string map_points_topic = "/hdl_graph_slam/map_points_" + std::to_string(vehicle_num);
        map_points_pubs.push_back(mt_nh.advertise<sensor_msgs::PointCloud2>(map_points_topic, 1, true));
      }
  ...
  private:
    ...
    bool manage_multi_vehicle_keyframe(int vehicle_num, Eigen::Isometry3d odom2map)
    {
      ...
      for (int i = 0; i < std::min<int>(keyframe_queues[vehicle_num].size(), max_keyframes_per_update); i++) // Keyframe queue에 있는 개수가 min보다 작으면 개수만큼만 진행
      {
        ...
        const auto &prev_keyframe = i == 0 ? mvvKFs[vehicle_num].back() : mvdKFs[vehicle_num][i - 1]; // 일단 i==0 일때 현재 키프레임과 이전 키프레임을 엮어줘야하니 prev_keyframe을 지정해주는 것으로 보임
        /* --------------------------------------------------------------------------- */
        ...
        static double stddev_x = private_nh.param<double>("odom_edge_stddev_x", 0.0l);
        static double stddev_q = private_nh.param<double>("odom_edge_stddev_q", 0.1);
        Eigen::MatrixXd inf = Eigen::MatrixXd::Identity(6, 6);
      ...
      mvdKFs[vehicle_num].erase( mvdKFs_new[vehicle_num].begin(),  mvdKFs_new[vehicle_num].begin() + num_processed + 1);
      return true;
    }
